...
RoutingExperiment::Run (int nSinks, double txp)
{
  ...
  AnimationInterface anim ("uji-" + m_protocolName + "uji.xml");
  anim.EnableIpv4RouteTracking (tr_name + "_" + m_protocolName + "uji_routes.xml", Seconds (0), Seconds (TotalTime), Seconds (5.0));
  ...
  flowmon->SerializeToXmlFile ((tr_name + "-" + m_protocolName + "uiji.flowmon").c_str(), false, false);
...
RoutingExperiment::SaveMetrics (Ptr<FlowMonitor> flowmon, FlowMonitorHelper *flowmonHelper)
{
  ...
  // Save to CSV
  std::ofstream out ("uji.csv", std::ios::app);
